...
export function IssueCard({ issue, memberCount, onJump }: Props) {
  ...
  return (
    <div className={cn("mb-2 flex gap-2 rounded-section border border-line bg-panel p-3", issue.resolved && "opacity-50")}>
      <span className="w-[3px] shrink-0 rounded-pill" style={{ background: color }} aria-hidden />
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className={cn("shrink-0 text-meta font-semibold", isHigh ? "text-bad" : "text-t3")}>
            {SEVERITY_LABEL[issue.severity]}
          </span>
          <span className="font-medium text-t1">{issue.title}</span>
          {issue.resolved && <span className="ml-auto text-meta text-ok">已解决</span>}
        </div>
        ...
        {(issue.severityDisagreement || issue.file == null) && (
          <div className="mt-1 flex flex-wrap gap-1.5 text-meta">
            {issue.severityDisagreement && (
              <span className="rounded-pill bg-status-human-bg px-1.5 py-0.5 text-status-human" title="不同审查者对这条意见的严重程度判断不一致，建议人工裁决">
                ⚠ 严重度存在分歧
              </span>
            )}
            {issue.file == null && (
              <span className="rounded-pill bg-raised px-1.5 py-0.5 text-t3" title="这条意见没有指向具体文件，无法直接对照代码，请人工核实">
                无文件定位，请人工核实
              </span>
            )}
          </div>
        ...
        {(reporters.length > 0 || issue.agreementCount > 1) && (
          <div className="mt-1 flex flex-wrap items-center gap-1.5 text-meta text-t3">
            {reporters.length > 0 && (
              <>
                <span>由</span>
                {reporters.map((agent) => (
            ...
            {issue.agreementCount > 1 && (
              <span className="rounded-pill bg-raised px-1.5 py-0.5 text-t2">
        ...
        {loc && (
          <button
            ...
            disabled={!onJump}
            title="跳到 Diff 对应位置"
            className="mt-1 font-mono text-meta text-link hover:underline disabled:no-underline"
          >
            {loc}
          </button>
        )}
        {issue.description && <p className="mt-2 whitespace-pre-wrap text-body leading-relaxed text-t2">{issue.description}</p>}
        {issue.suggestedAction && (
          <div className="mt-2">
            <button type="button" onClick={() => setOpen((o) => !o)} className="flex items-center gap-1 text-meta text-t3 hover:text-t2">
              <ChevronRight className={cn("size-3.5 transition-transform", open && "rotate-90")} /> 建议动作
            </button>
            {open && (
              <p className="mt-1 whitespace-pre-wrap rounded-control bg-app px-3 py-2 text-body text-t2">{issue.suggestedAction}</p>
            )}
          </div>
        )}
      </div>
