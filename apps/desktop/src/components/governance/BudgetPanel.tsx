...
export function BudgetPanel({ usage, onAdjust }: { usage: BudgetUsage; onAdjust?: () => void }) {
  ...
  return (
    <section className="rounded-[var(--radius-panel)] border border-line bg-panel/60 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          {view.level === "blocked" ? (
            ...
            <CircleAlert className="size-4 text-human" aria-hidden />
          ) : null}
          <h2 className="font-semibold">预算</h2>
          <span
            className={cn(
              "text-[12px]",
              view.level === "blocked" ? "text-bad" : view.level === "warn" ? "text-human" : "text-t2"
            )}
          >
            {view.headline}
          </span>
        ...
        {onAdjust && view.level !== "ok" && (
          <button type="button" onClick={onAdjust} className="text-[12px] text-run hover:underline">
            调整预算上限
          </button>
        )}
      </div>

      {note && <p className="mt-1.5 text-[12px] text-t3">{note}</p>}
      ...
      {view.exceeded && (
        <p className="mt-3 rounded-md border border-human/50 bg-human-bg px-3 py-2 text-[12px] text-t1">
...
function MetricCard({ metric }: { metric: BudgetMetric }) {
  ...
  return (
    <div className="rounded-[var(--radius-panel)] border border-line bg-app/40 p-3">
      <div className="flex items-baseline justify-between gap-2">
        <span className="text-[12px] font-medium">{metric.label}</span>
        <span className={cn("text-[12px]", unknown ? "text-t3" : "text-t2")}>
          {metric.display} / {metric.limitDisplay}
        </span>
      </div>

      <div className="mt-2 flex h-1.5 overflow-hidden rounded-full bg-line" role="img" aria-label={`${metric.label}用量`}>
        {unknown ? (
          ...
          <>
            <span
              className={cn(
                "h-full",
                metric.level === "blocked" ? "bg-bad" : metric.level === "warn" ? "bg-human" : "bg-run"
              )}
              style={{ width: `${metric.settledPercent}%` }}
            />
            {/* 已预留但尚未结算，与已结算区分开。 */}
            <span
              className="h-full bg-run/35 bg-[repeating-linear-gradient(45deg,transparent_0_3px,rgba(0,0,0,0.08)_3px_6px)]"
      ...
      <div className="mt-1.5 flex items-center gap-1 text-[11px] text-t3">
        {unknown && <CircleQuestionMark className="size-3 shrink-0" aria-hidden />}
        <span>{metric.enforcementLabel}</span>
      </div>
      {metric.note && <p className="mt-1 text-[11px] leading-relaxed text-t3">{metric.note}</p>}
    </div>
  );
}
