  ...
  return (
    <div className="space-y-3 text-meta">
      {value.byteOnlyChange && (
        <div className="rounded-control border border-warn/35 bg-warn/5 p-3 text-t2">只检测到注释、空白或格式字节变化；权限语义没有变化，但 SHA 授权仍已失效，需要重新确认。</div>
      )}
      {value.changes.length > 0 && (
        <div className="rounded-control border border-line bg-raised p-3">
          <div className="flex items-center justify-between gap-2 font-medium text-t1">
            ...
            {highRisk > 0 && <span className="inline-flex items-center gap-1 text-caution"><AlertTriangle className="size-3.5" /> {highRisk} 项权限相关</span>}
          </div>
          <div className="mt-2 space-y-2">
            {value.changes.map((change) => (
              <div key={change.path} className="rounded border border-line bg-app/55 p-2">
                <div className="flex items-center gap-2">
                  <span className={change.highRisk ? "font-medium text-caution" : "font-medium text-t1"}>{configPathLabel(change.path)}</span>
                  <span className="text-micro text-t3">{CONFIG_CHANGE_LABEL[change.kind]}</span>
                </div>
                {change.before !== null && <ValueLine icon={<Minus className="size-3" />} tone="text-caution" value={change.before} />}
        ...
        <div className="mt-2 space-y-2">
          {value.validationCommands.length ? value.validationCommands.map((command) => (
            <div key={command.name} className="rounded bg-app/60 p-2">
              ...
              <code className="mt-1 block break-all text-meta text-t1">{command.argv.join(" ")}</code>
            </div>
          ...
          <div className="text-meta text-t3">环境锁定：{value.lockEnvironment ? "开启" : "关闭"} · 密闭复现：{value.hermetic ? "开启" : "关闭"}</div>
        </div>
      </div>
    </div>
  );
...
function ValueLine({ icon, tone, value }: { icon: ReactNode; tone: string; value: string }) {
  return <div className={`mt-1 flex items-start gap-1 font-mono text-micro ${tone}`}>{icon}<pre className="min-w-0 whitespace-pre-wrap break-all">{displayConfigValue(value)}</pre></div>;
}
