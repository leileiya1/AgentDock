...
export function PreflightProgress({ roles }: { roles: PendingPreflightRole[] }) {
  return (
    <div className="rounded-control border border-status-running/35 bg-status-running/5 px-3 py-2.5" role="status" aria-live="polite">
      <div className="flex items-center gap-2 text-body font-medium text-t1">
        <LoaderCircle className="size-4 animate-spin text-status-running" aria-hidden />
        正在并行检测 Provider
      </div>
      <div className="mt-2 flex flex-col gap-1.5">
        {roles.map((role) => (
          <div key={role.role} className="grid grid-cols-[3rem_minmax(0,1fr)] gap-2 text-meta leading-5">
            ...
            <div className="flex flex-wrap gap-x-3 gap-y-1">
              {role.providers.map((provider) => (
                <span key={provider.key} className="inline-flex items-center gap-1.5 text-t2">
                  <span className="size-1.5 rounded-circle bg-status-running animate-pulse-dot" aria-hidden />
                  {provider.label} · 检测中
                </span>
              ))}
            </div>
      ...
      <p className="mt-1.5 text-meta text-t3">全部返回后再创建任务；不可用项会保留具体原因。</p>
    </div>
  );
}
...
export function PreflightReportBody({ view }: { view: PreflightView }) {
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-start gap-2 rounded-control border border-caution/50 bg-caution-bg px-3 py-2 text-body leading-5 text-t1">
      ...
      <p className="text-meta leading-5 text-t3">
  ...
  return (
    <div className="rounded-control border border-line bg-app/60 p-3">
      <div className="mb-2 flex items-center justify-between gap-3">
        <div className="text-body font-medium text-t1">
        ...
        {role.ready ? (
          ...
          <span className="flex items-center gap-1 text-meta text-caution">
      ...
      <ul className="flex flex-col gap-1.5">
        {role.providers.map((provider) => (
          <li key={provider.key} className="flex items-start gap-2 text-meta leading-5">
            <span
              className={`mt-1.5 size-2 shrink-0 rounded-circle ${provider.available ? "bg-ok" : "bg-caution"}`}
              aria-hidden
            />
