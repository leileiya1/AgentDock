...
export function ProjectConfigTrustPanel({ projectId }: { projectId: string }) {
  ...
  if (!value.exists) {
    return (
      <div className="rounded-section border border-line bg-app p-3 text-meta text-t3">
  ...
  return (
    <>
      <div className={`rounded-section border p-3 ${value.trusted ? "border-ok/40 bg-app" : "border-caution/50 bg-caution-bg"}`}>
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2 font-semibold">
              {value.trusted ? <ShieldCheck className="size-4 text-ok" /> : <ShieldAlert className="size-4 text-caution" />}
              仓库配置权限
            </div>
        ...
        <div className="mt-2 break-all font-mono text-meta text-t3">SHA-256 {value.sha256}</div>
        {!!value.validationSteps.length && <div className="mt-2 text-meta text-t2">验证步骤：{value.validationSteps.join("、")}</div>}
        {!!value.extraAllowedCommands.length && <div className="mt-1 text-meta text-caution">额外命令权限：{value.extraAllowedCommands.join("、")}</div>}
        {!value.trusted && value.changes.length > 0 && <div className="mt-1 text-meta text-caution">检测到 {value.changes.length} 项配置变化，其中 {value.changes.filter((change) => change.highRisk).length} 项会改变执行或复现权限。</div>}
      </div>
      ...
      >
        <p className="mb-3 text-body text-t2">批准后，AgentFlow 可以执行下列仓库命令并使用所列复现配置。文件只要变化一个字节，授权就会自动失效。</p>
        <ProjectConfigChangePreview value={value} />
        <div className="mt-3 break-all font-mono text-micro text-t3">上次 {value.previousApprovedSha256 ?? "无"}<br />当前 {value.sha256}</div>
      </Dialog>
    </>
  );
}
