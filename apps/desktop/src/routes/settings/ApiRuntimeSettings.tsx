  ...
  return (
    <div className="rounded-section border border-line bg-app p-3">
      <button type="button" className="flex w-full items-center justify-between text-left" onClick={() => setOpen((value) => !value)}>
        <div><div className="text-body font-semibold">API 高级运行设置</div><div className="mt-0.5 text-meta text-t3">模型、Base URL 与费用预算所需的价格快照</div></div>
      ...
      {open && <div className="mt-3 flex flex-col gap-3 border-t border-line pt-3">
        ...
        <p className="text-meta leading-relaxed text-t3">价格由你按当前供应商账单填写并随项目保存；输入和输出必须同时填写。AgentFlow 不内置可能过期的价格，API 返回真实费用时优先采用真实值。</p>
      </div>}
    </div>
  );
}
